...
var processCommands = {
  ...
  textures: function () {
    // Create directory that will hold all files
    var processedTexturesDir = path.join('data/converted', metadata.version, 'textures/cards');
    var rawTexturesDir = path.join('data/raw', metadata.version, 'Data/cardtextures0/CAB-cardtextures0/Texture2D');

    fs
      .readdirSync(rawTexturesDir)
      .forEach(function(file) {
        gm(path.join(rawTexturesDir, file))
          .options({imageMagick: true})
          .write(path.join(processedTexturesDir, file), function (err) {
            if (err) throw err;
            console.log('Converted texture: ' + file);
            console.log('Saved to: ' + path.join(processedTexturesDir, file));
            console.log('---------');
          });
      });
  }
